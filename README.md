...
[2. Reporting Installation or Usage Problems](#2-reporting-installation-or-usage-problems)

[3. Compiler Flags](#3-compiler-flags)

[4. Alternate Channels and Devices](#3-alternate-channels-and-devices)

[5. Alternate Process Managers](#5-alternate-process-managers)

[6. Alternate Configure Options](#6-alternate-configure-options)

[7. Testing the MPICH installation](#7-testing-the-mpich-installation)

[8. Fault Tolerance](#8-fault-tolerance)

[9. Developer Builds](#9-developer-builds)

[10. Multiple Fortran compiler support](#10-multiple-fortran-compiler-support)

[11. ABI Compatibility](#11-abi-compatibility)

[12. Capability Sets](#12-capability-sets)
...
using the environment variable MPIR_CVAR_OFI_USE_PROVIDER, which will also
do some optimization for the requested provider.
...
the environment variable:
    ...
    % export MPIR_CVAR_OFI_ENABLE_TAGGED=0

For some configuration options (in particular, MPIR_CVAR_OFI_ENABLE_TAGGED and
MPIR_CVAR_OFI_ENABLE_RMA), a default build of MPICH using the CH4 OFI netmod
will fail if these options are not enabled. This is so that, for performance,
MPICH can assume that some commonly available features are enabled by the OFI
provider. If using a provider that does not support FI_TAGGED, FI_RMA, or
FI_DELIVERY_COMPLETE, a special configure flag must be provided to build in
support for workarounds for such features. Compile with --enable-legacy-ofi
...
Deploy MPICH to Azure Cloud using the following "Deploy to Azure" button. It will create a VM cluster with a head node and a set of compute nodes.

You can select the HPC image to get pre-built HPC image for the compute nodes and load mpich environment module using:
...
The above deployment creates an [Azure VM Scaleset](https://docs.microsoft.com/azure/virtual-machine-scale-sets/overview) of requested size.
Head node hosts NFS server and mounts the ```/home``` directory for the compute nodes.

You can select either HPC image (pre-installed MPI libraries, along with a few OS optimizations enabled) or regular image (vanilla OS image).
